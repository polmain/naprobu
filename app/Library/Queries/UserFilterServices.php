        ...
        $users = User::with([
            ...
            ->when( !empty($filters->filter['status']), function ($query) use ($filters){
                ...
                $query->whereHas('children', function($q) use ($filters){
                    $q->where('birthday', '>=',  Carbon::now()->addYear($filters->filter['child_old_min'])->format('Y-m-d'));
                });
            })->when( !empty($filters->filter['child_old_max']), function ($query) use ($filters){
                $query->whereHas('children', function($q) use ($filters){
                    $q->where('birthday', '<=',  Carbon::now()->addYear($filters->filter['child_old_max'])->format('Y-m-d'));
                });
