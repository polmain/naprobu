...
@section('content')
    <div class='row'>
        <form action="{{route('adm_comment_save',['comment_id'=>$comment->id])}}" method="post" enctype="multipart/form-data" class="validation-form">
            {{ csrf_field() }}
            <div class='col-md-9'>
                <!-- Box -->
            ...
            <div class="col-md-3">
                ...
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        ...
                        <button type="button" class="btn btn-block btn-danger btn-lg" onclick="document.location.href='/admin/reviews/comments/';">Отмена</button>
                    </div><!-- /.box-body -->
                </div><!-- /.box -->
                <!-- Box -->
                <!-- Box -->
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        ...
                        <div class="form-group">
                            <label>
                                <input type="checkbox" class="minimal-red" name="isImportant" value="true"{{($comment->isImportant)?" checked=checked":""}}>
                                Важное
                            </label>
                        </div>
                        <div class="form-group row">
                        ...
                        <div class="form-group row">
                            <div class="col-md-12">
                                ...
                                <select id="project" name="project" class="form-control" disabled>
                                    <option  value="{{$comment->review->id}}" selected=selected >{{$comment->review->text}}</option>
                                </select>
                            </div>
                        </div>
                        <div class="form-group row">
                            <div class="col-md-12">
                                <label>Проект</label>
                                <select id="project" name="project" class="form-control" disabled>
                                    <option  value="{{$comment->review->subpage->project->id}}" selected=selected >{{$comment->review->subpage->project->name}}</option>
                                </select>
                            </div>
                        </div>
                        <div class="form-group row">
                            <div class="col-md-12">
                                <label>Подстраница проекта</label>
                                <select id="subpage" name="subpage" class="form-control required" disabled>
                                    <option  value="{{$comment->review->subpage->id}}" selected=selected >{{$comment->review->subpage->name}}</option>
                                </select>
                            </div>
                        </div>
                    </div><!-- /.box-body -->
